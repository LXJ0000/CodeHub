...
func Init() {
	...
	mLog.SetReportCaller(conf.Conf.LogConfig.ShowLine) //开启返回函数名和行号
	mLog.SetFormatter(&LogFormatter{})                 //设置自己定义的Formatter
	level, err := logrus.ParseLevel(conf.Conf.LogConfig.Level)
	if err != nil {
...
func DefaultLoggerInit() {
	...
	logrus.SetReportCaller(conf.Conf.LogConfig.ShowLine) //开启返回函数名和行号
